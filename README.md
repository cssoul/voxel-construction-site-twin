...
src/
  components/            Vue UI 组件
    PanelLeft.vue        左侧面板：一天的时间(默认正午/滑杆/黎明-正午-黄昏-夜晚/自动流逝)、天气(晴/暴雨)、实体按钮(分组列表)
  ...
  scene/
    ...
    Weather.ts             天气系统（暴雨渐变/地面湿润/尘土&雨粒子参数）
...
- 左侧面板：时间滑杆与黎明/正午/黄昏/夜晚预设（默认正午）、自动流逝开关、晴/暴雨切换、按分组的实体按钮（点击选中并让相机聚焦飞行）
